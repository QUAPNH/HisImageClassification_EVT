...
class LabelSmoothingCrossEntropy1(nn.Module):
    ...
    def _compute_losses(self, x,q, target):
        log_prob = F.log_softmax(x, dim=-1)
        nll_loss = -log_prob.gather(dim=-1, index=target.unsqueeze(1))
        nll_loss = nll_loss.squeeze(1)
        smooth_loss = -log_prob.mean(dim=-1)
        loss = self.confidence * nll_loss + self.smoothing * smooth_loss + q
        return loss

    def forward(self, x, q, target):
        return self._compute_losses(x, q,target).mean()