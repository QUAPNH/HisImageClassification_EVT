...
from .EA import TransformerEncoderLayer
...
class TransformerClassifier(nn.Module):
    def __init__(self,
        ...
        if positional_embedding != 'none':
            if positional_embedding == 'learnable':
                self.fc_c = nn.Conv1d(embedding_dim, embedding_dim, 1, 1, bias=True)
                self.theta_x_conv = nn.Sequential(nn.Conv1d(embedding_dim, embedding_dim, 1, 1, bias=True), nn.BatchNorm1d(embedding_dim), nn.ReLU())
                self.fc_a = nn.Conv1d(embedding_dim*2, embedding_dim, 1, 1, bias=True)
                # self.positional_emb = nn.Parameter(torch.zeros(1, sequence_length, embedding_dim),
                #                                    requires_grad=True)
                # nn.init.normal_(self.positional_emb, std=0.2)
            else:
    ...
    def forward(self, x):
        # if self.theta_x_conv is None and x.size(1) < self.sequence_length:
        ...
        if self.theta_x_conv is not None:
            z = x
            z = z.transpose(-2,-1)

            c = self.fc_c(z)
            xx = self.theta_x_conv(z)
            xx = torch.cat([c * torch.cos(xx), c * torch.sin(xx)], dim=1)
            xx = self.fc_a(xx)
            xx = xx.transpose(-2, -1)
            x  = x + xx
        ...
        # x = torch.matmul(F.softmax(self.gap(x)).transpose(-1, -2), x).squeeze(-2)
